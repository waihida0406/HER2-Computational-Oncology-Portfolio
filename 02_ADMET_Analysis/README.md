# 🧪 Project 2: ADMET Screening of *Ventilago madraspatana* Phytochemicals

##  Status

✅ Completed

---

##  Background

Drug candidates must possess suitable pharmacokinetic properties before they can be considered for further development. ADMET (Absorption, Distribution, Metabolism, Excretion, and Toxicity) analysis helps predict whether a compound is likely to be safe, bioavailable, and drug-like.

In this project, phytochemicals obtained from *Ventilago madraspatana* were evaluated using computational ADMET prediction to identify compounds with favorable drug-like properties prior to molecular docking.

---

##  Objective

To evaluate the pharmacokinetic properties and drug-likeness of selected phytochemicals from *Ventilago madraspatana* using SwissADME and identify compounds suitable for molecular docking against the HER2 protein.

---

##  Compound Source

- IMPPAT Database
- PubChem Database

---

## Software & Databases

- SwissADME
- IMPPAT
- PubChem

---

##  Workflow

1. Retrieved phytochemicals reported for *Ventilago madraspatana* from the IMPPAT database.
2. Obtained compound information and SMILES notation from PubChem.
3. Entered SMILES into SwissADME.
4. Evaluated pharmacokinetic properties.
5. Assessed drug-likeness using Lipinski's Rule of Five.
6. Compared compounds based on ADMET properties.
7. Selected compounds with favorable profiles for molecular docking.

---

##  Parameters Evaluated

### Absorption
- Gastrointestinal (GI) Absorption
- Water Solubility

### Distribution
- Blood-Brain Barrier (BBB) Permeability

### Metabolism
- CYP450 Enzyme Prediction

### Drug-likeness
- Lipinski Rule of Five
- Bioavailability Score

### Medicinal Chemistry
- PAINS Alerts
- Lead-likeness

---

##  Outcome

- Successfully evaluated the pharmacokinetic properties of selected phytochemicals.
- Identified compounds with good drug-likeness and favorable ADMET profiles.
- Eliminated compounds with poor pharmacokinetic characteristics.
- Selected promising phytochemicals for molecular docking studies against HER2.

---

##  Files Included

- ADMET prediction results
- SwissADME reports
- Compound list
- Selected compounds for docking

---

##  Skills Demonstrated

- Drug-likeness prediction
- Pharmacokinetic analysis
- Virtual screening
- Computational drug discovery
- Database mining
- SwissADME analysis

---

## 📚 References

1. SwissADME: http://www.swissadme.ch
2. IMPPAT Database
3. PubChem Database
